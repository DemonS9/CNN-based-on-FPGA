...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1_out_np = np.zeros((1, 3, 24, 24))    #第一层卷积层输出
        self.mp1_out_np = np.zeros((1, 3, 12, 12))      #第一层的池化输出

        #self.conv2_out_np = np.zeros((1, 3, 8, 8))      #第二层的卷积层输出
        #self.mp2_out_np = np.zeros((1, 3, 4, 4))        #第二层池化层输出

        self.conv2_out_np = np.zeros((1, 3, 10, 10))  # 第二层的卷积层输出
        self.mp2_out_np = np.zeros((1, 3, 5, 5))  # 第二层池化层输出

        self.fc_in_np = np.zeros((1, 75))               #全连接层输入
        self.fc_out_np = np.zeros((1, 10))              #全连接层输出
        
        # 1st Convolution Layer
        # Image Input Shape -> (28, 28, 1)  //输入图像大小
        # Convolution Layer -> (24, 24, 3)  //卷积层
        # Pooling Max Layer -> (12, 12, 3)  //最大池化层
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=3, kernel_size=5)     #维度为1   卷积产生3通道   卷积核尺寸为5
        
        # 2nd Convolution Layer
        # Image Input Shape -> (12, 12, 3)
        # Convolution Layer -> (8, 8, 3)
        # pooling Max Layer -> (4, 4, 3)
        #self.conv2 = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=5,stride=1)     #维度为3   卷积产生3通道   卷积核尺寸为3

        # Image Input Shape -> (12, 12, 3)
        # Convolution Layer -> (10, 10, 3)
        # pooling Max Layer -> (5, 5, 3)
        self.conv2 = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3)  #维度为3    卷积产生3通道   卷积核尺寸改为3
...
orig_output_calc_1 = np.zeros((3,10,10))
orig_output_calc_2 = np.zeros((3,10,10))
orig_output_calc_3 = np.zeros((3,10,10))
orig_output_calc = np.zeros((3,10,10))

for c in range(3):
    for i in range(10):
        for j in range(10):
            # MAC
            orig_output_calc_1[c][i][j] += (orig_input_1[i:i+3, j:j+3] * orig_weight[c][0]).sum()
            orig_output_calc_2[c][i][j] += (orig_input_2[i:i+3, j:j+3] * orig_weight[c][1]).sum()
            orig_output_calc_3[c][i][j] += (orig_input_3[i:i+3, j:j+3] * orig_weight[c][2]).sum()
            orig_output_calc[c][i][j] = orig_output_calc_1[c][i][j] + orig_output_calc_2[c][i][j] + orig_output_calc_3[c][i][j] + orig_bias[c]
...
output_calc_1 = np.zeros((3,10,10))
output_calc_2 = np.zeros((3,10,10))
output_calc_3 = np.zeros((3,10,10))
output_calc = np.zeros((3,10,10))

for c in range(3):
    for i in range(10):
        for j in range(10):
            # MAC
            output_calc_1[c][i][j] += (_input_1[i:i+3, j:j+3] * weight[c][0]).sum()
            output_calc_2[c][i][j] += (_input_2[i:i+3, j:j+3] * weight[c][1]).sum()
            output_calc_3[c][i][j] += (_input_3[i:i+3, j:j+3] * weight[c][2]).sum()
            output_calc[c][i][j] = output_calc_1[c][i][j] + output_calc_2[c][i][j] + output_calc_3[c][i][j] + bias[c] * 128
